...
而关键路径上的信息有：线程执行时间（节点）、线程同步等待时间（边）、调度时间（边）。

futex等待时间一般与cpu性能无关，调度时间亦然。因此**寻找关键路径时应该考虑三者**，但**计算权重时应该仅考虑执行时间**。

到这一步，我大胆再引进两个假设:

- 计算量固定假设：关键路径上的线程不是贪婪地抢占cpu时间，只是需要固定的计算量
- 边际收益最大化假设：关键路径上的线程节点需要的cpu时间越大，给它更多性能预算就越划算

所以，可以尝试将线程在对应集簇的执行时间作为权重来分配FAS闭环控制出的性能预算。
